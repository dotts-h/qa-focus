...
// The explorer/codifier run this module through the `tsx` (esbuild) loader, which decorates the
// named functions inside `snapshotPage`'s `page.evaluate` callback with `__name(...)` — esbuild's
// keep-names helper. That helper exists only in THIS module's scope, not in the page's browser
// context where the callback actually executes, so an unguarded `page.evaluate(() => { … named fns … })`
// throws `ReferenceError: __name is not defined` on every no-CDP surface (Electron + friends). It
