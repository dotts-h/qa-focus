...
| `createGatedSession({cli,model,tools,stepBudget,recency,quiet})` | → `{ session, client, toolNames, flushStream, detachStream }`; the hard leash (cage + deny + budget) + the live run-stream tap (#0013) | **stable** — the injection defense | `src/harness.mts` · [ADR 0002](adr/0002-extract-gated-session-harness.md) |
