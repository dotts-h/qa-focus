...
| Model selection (#0017) | `resolveModel(models, requested) → { ok:true, model? } \| { ok:false, error }` (exact-id, fail-loud on unknown) + `formatModelList(models)`; `createGatedSession({model})` validates against `client.listModels()` before opening the session — an unknown `--model` throws, never a silent fallback. CLI: `qa-focus models` / `--list-models`, `--model <id>`, `--quiet` (headless). | **stable** | `src/models.mts` · `bin/{models,qa-focus}.mts` |
| Streamable REPL (#0016) | `createLineWriter(emit)` buffers partial stream chunks → complete lines; `writeAbovePrompt(surface, text)` does clear→cursor0→write→redraw on an injected `PromptSurface`. `createGatedSession({streamWrite})` routes the live stream to a sink (default stdout); interactive uses a readline-aware writer to render lines above a live prompt (TTY only). | **stable** | `src/repl.mts` · `bin/interactive.mts` · [ADR 0008](adr/0008-streamable-repl-on-readline-not-ink.md) |
| VS Code participant (#0018) | `parseChatRequest(prompt) → { mode: explore\|codify\|help, url?, goal?, argv[], error? }` (pure) maps an `@qa-focus` chat prompt to a qa-focus CLI argv; the extension registers a chat participant that owns the loop and streams the run — no LM-tool, no MCP (ADR 0007). | **stable** | `vscode/src/{request,extension}.ts` · [ADR 0007](adr/0007-vscode-surface-chat-participant.md) |
