...
mirrored to the github tracker so the team gets the familiar UI, labels, and notifications.
...
- **Mirror:** one github issue per file, kept in sync by the recipe's sync script. The local
...
3. **Mirror to github:** run the recipe's sync script — `scripts/sync-github.sh` on GitHub
...
greppable); the github mirror is the team's familiar view. Exploratory findings from
