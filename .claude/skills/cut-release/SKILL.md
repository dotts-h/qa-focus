...
3. **The build step is real, not a stub.** The workflow's build step must actually produce artifacts
   in `dist/` (`echo "TODO(backfill) build $VERSION artifacts into dist/" > dist/BUILD_TODO.txt`). If it still writes a `BUILD_TODO.txt` placeholder, the
...
- The release for the exact tag exists with `tag_name == "<tag>"` (NOT the branch name), and the
  asset names embed the version (`qa-focus-<tag>-<os>-<arch>`) alongside `checksums.txt`.
